...
public class AdviceWindowScript : MonoBehaviour
{
	...
	public void UpdateText()
	{
		...
		else if (Selected == 4)
		{
			...
			if (ShowOffID == 1)
			{
				...
				AdviceLabel.text = "Your suitor's courage is ''Level " + DatingGlobals.GetSuitorTrait(1) + "''. " + text;
			}
			else if (ShowOffID == 2)
			{
				...
				AdviceLabel.text = "Your suitor's intelligence is ''Level " + DatingGlobals.GetSuitorTrait(2) + "''. " + text;
			}
			else if (ShowOffID == 3)
			{
				...
				AdviceLabel.text = "Your suitor's strength is ''Level " + DatingGlobals.GetSuitorTrait(3) + "''. " + text;
			}
		}
		else if (Selected == 5)
		{
		...
		StatsLabel.text = "SUITOR STATS\n\nCOURAGE: " + DatingGlobals.GetSuitorTrait(1) + "\n\nINTELLIGENCE: " + DatingGlobals.GetSuitorTrait(2) + "\n\nSTRENGTH: " + DatingGlobals.GetSuitorTrait(3);
	}
	...
	private void CalculateMultiplier()
	{
		...
		{
			AttractionText[1] = "Your rival is indifferent towards her suitor's hairstyle.";
		}
		...
		if (rivalBookBag.DesiredHairColor == "" && Suitor.Cosmetic.HairColor != "SolidBlack")
		{
		...
		{
			AttractionText[8] = "You could provide the suitor with better advice if you purchased romance manga in town and read it at night.";
		}
	...
	public void CalculateLove()
	{
		...
		{
			if (DatingGlobals.GetSuitorTrait(ShowOffID) > DatingGlobals.GetTraitDemonstrated(ShowOffID))
			{
				Affection += Multiplier;
	...
	public void SaveTopicsAndCompliments()
	{
		...
		for (int l = 1; l < 5; l++)
		{
			CollectibleGlobals.SetGiftPurchased(l + 5, GiftsInPossession[l]);
			CollectibleGlobals.SetGiftGiven(l, GiftsGiven[l]);
		}
