...
public class PhotoGalleryScript : MonoBehaviour
{
	...
	private void UpdatePhotoSelection()
	{
		if (Input.GetButtonDown("A"))
		{
			...
			else if (Photographs[CurrentIndex].mainTexture != NoPhoto && PlayerGlobals.GetBullyPhoto(CurrentIndex) > 0)
			{
		...
		if (Input.GetButtonDown("X"))
		{
			...
			if (Photographs[currentIndex].mainTexture != NoPhoto)
			{
				...
				PlayerGlobals.SetPhoto(currentIndex, value: false);
				PlayerGlobals.SetSenpaiPhoto(currentIndex, value: false);
				TaskGlobals.SetGuitarPhoto(currentIndex, value: false);
				TaskGlobals.SetKittenPhoto(currentIndex, value: false);
		...
		else if (Input.GetButtonDown("Y") && PlayerGlobals.GetSenpaiPhoto(CurrentIndex))
		{
			int currentIndex2 = CurrentIndex;
			PlayerGlobals.SenpaiShots--;
			Yandere.Inventory.SenpaiShots--;
			PlayerGlobals.SetSenpaiPhoto(currentIndex2, value: false);
	...
	public IEnumerator GetPhotos()
	{
		...
		for (int ID = 1; ID < 26; ID++)
		{
			...
			else
			{
				...
				PlayerGlobals.SetPhoto(ID, value: false);
			}
		}
	...
	public void UpdateButtonPrompts()
	{
		if (NamingBully)
		{
			if (Photographs[CurrentIndex].mainTexture != NoPhoto && PlayerGlobals.GetBullyPhoto(CurrentIndex) > 0)
			{
		...
		else if (!Viewing)
		{
			...
			if (!Corkboard)
			{
				PromptBar.Label[3].text = (PlayerGlobals.GetSenpaiPhoto(currentIndex) ? "Use" : string.Empty);
			}
			else
			{
