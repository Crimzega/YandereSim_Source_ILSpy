...
public class NemesisScript : MonoBehaviour
{
	...
	private void SpecialEffect()
	{
		...
		else if (EffectPhase == 2 && component["f02_knifeLowSanityA_00"].time > 4.16666651f)
		{
			Object.Instantiate(BloodEffect, Knife.transform.position + Knife.transform.forward * 0.1f, Quaternion.identity);
			EffectPhase++;
		}
	...
	private void HideObjects()
	{
		...
		punkAccessories = Student.Fingerfood;
		foreach (GameObject gameObject2 in punkAccessories)
		{
			if (gameObject2 != null)
			{
				gameObject2.SetActive(value: false);
