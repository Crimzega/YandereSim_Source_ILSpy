						...
						if (Carried || InSight)
						{
							...
							for (ID = 0; ID < 4; ID++)
							{
								if (ButtonActive[ID])
								{
									...
									if (Vector3.Angle(Yandere.MainCamera.transform.forward, Yandere.MainCamera.transform.position - base.transform.position) > 90f)
									{
										if (Local)
										{
											Vector2 vector3 = MainCamera.WorldToScreenPoint(base.transform.position + base.transform.right * OffsetX[ID] + base.transform.up * OffsetY[ID] + base.transform.forward * OffsetZ[ID]);
										...
										{
											vector2 = MainCamera.WorldToScreenPoint(base.transform.position + new Vector3(OffsetX[ID], OffsetY[ID], OffsetZ[ID]));
