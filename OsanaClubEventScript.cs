...
public class OsanaClubEventScript : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (Clock.HourTime > 13.5f || EventStudent[1].WitnessedCorpse || EventStudent[2].WitnessedCorpse || EventStudent[1].Alarmed || EventStudent[2].Alarmed || EventStudent[1].Dying || EventStudent[2].Dying || EventStudent[1].Splashed || EventStudent[1].Dodging || Clock.Police.EndOfDay.gameObject.activeInHierarchy)
		{
			EndEvent();
			return;
		}
			...
			if (!Spoken)
			{
				...
				if (EventSpeaker[EventPhase] == 1 && EventPhase > 7 && EventPhase < 33 && EventPhase != 24 && num < 10f)
				{
					if (EventPhase == 30)
					{
						...
						ConversationGlobals.SetTopicLearnedByStudent(ClubIDs[EventPhase], 17, value: true);
						return;
					}
					...
					Yandere.NotificationManager.DisplayNotification(NotificationType.Opinion);
					ConversationGlobals.SetTopicLearnedByStudent(ClubIDs[EventPhase], 11, value: true);
				}
				return;
			}
	...
	public void CheckForRooftopConvo()
	{
		...
		{
			Debug.Log("Raibaru is not null, and her CurrentAction is " + StudentManager.Students[10].CurrentAction);
			if (StudentManager.Students[10].CurrentAction == StudentActionType.Follow)
			{
