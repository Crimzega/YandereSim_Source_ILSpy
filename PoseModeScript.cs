		...
		if (Show)
		{
			...
			else
			{
				...
				if (Input.GetButtonDown("A"))
				{
					if (Selected == 2)
					{
						PoseSerializer.SerializePose(Student.Cosmetic, Student.transform, string.Concat(SaveSlot));
					...
					else if (Selected == 3)
					{
						...
						PoseSerializer.DeserializePose(Student.Cosmetic, Student.transform, string.Concat(SaveSlot));
					}
				}
				if (Input.GetButtonDown("B"))
				{
