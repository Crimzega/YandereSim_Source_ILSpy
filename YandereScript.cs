...
public class YandereScript : MonoBehaviour
{
				...
				if (Schoolwear > 0 && !WearingRaincoat)
				{
					...
					if (!ClubAttire)
					{
						if (CurrentUniformOrigin == 1)
						{
							...
							Debug.Log("One of the original uniforms has become bloody. There are now " + StudentManager.OriginalUniforms + " clean original uniforms in the school.");
						}
						else
						{
							StudentManager.NewUniforms--;
							Debug.Log("One of the new uniforms has become bloody. There are now " + StudentManager.NewUniforms + " clean original uniforms in the school.");
						}
					}
				}
			}
	...
	public Vector3 HeadPosition => new Vector3(base.transform.position.x, Hips.position.y + 0.2f, base.transform.position.z);
	...
	private void Start()
	{
		...
		YandereFilter = components[0];
		SenpaiFilter = components[1];
	...
	private void UpdateMovement()
	{
		if (CanMove)
		{
			...
			if (!NearSenpai)
			{
				if (!Input.GetButton("A") && !Input.GetButton("B") && !Input.GetButton("X") && !Input.GetButton("Y") && !StudentManager.Clock.UpdateBloom && (Input.GetAxis("LT") > 0.5f || Input.GetMouseButton(1)))
				{
					...
					if (!Aiming)
					{
						...
						else
						{
							...
							Smartphone.nearClipPlane = 0.12f;
						}
				...
				if (!Aiming && !Accessories[9].activeInHierarchy && !Accessories[16].activeInHierarchy && !Pod.activeInHierarchy && PermitLaugh > 1f && !Chased && !Input.GetButton("A"))
				{
					...
					if (Input.GetButtonUp("RB"))
					{
						if (CanCloak && !WearingRaincoat)
						{
							...
							{
								PauseScreen.NewSettings.QualityManager.UpdateOutlinesAndRimlight();
								SetUniform();
				...
				bool flag = false;
				if (StudentManager.Eighties && Aiming)
				{
					flag = true;
				}
				if (!flag)
			...
			if (Aiming)
			{
				if (!StudentManager.Eighties || (StudentManager.Eighties && Club == ClubType.Photography))
				{
					if (!StudentManager.Eighties)
					{
						if (!RivalPhone && Input.GetButtonDown("A"))
						{
							Selfie = !Selfie;
							UpdateSelfieStatus();
						}
		...
		if (Chased && !Sprayed && !Attacking && !Dumping && !StudentManager.PinningDown && !DelinquentFighting && !ShoulderCamera.HeartbrokenCamera.activeInHierarchy)
		{
			if (Pursuer != null)
			{
		...
		if (Digging)
		{
			if (DigPhase == 1)
			{
				if (CharacterAnimation["f02_shovelDig_00"].time >= 1.66666663f)
				{
					MyAudio.volume = 1f;
					MyAudio.clip = Dig;
					MyAudio.Play();
			...
			{
				if (CharacterAnimation["f02_shovelDig_00"].time >= 5.66666651f)
				{
					MyAudio.volume = 1f;
					MyAudio.Play();
					DigPhase++;
		...
		if (Burying)
		{
			if (DigPhase == 1)
			{
				if (CharacterAnimation["f02_shovelBury_00"].time >= 2.16666675f)
				{
					MyAudio.volume = 1f;
					MyAudio.clip = Dig;
					MyAudio.Play();
			...
			{
				if (CharacterAnimation["f02_shovelBury_00"].time >= 4.66666651f)
				{
					MyAudio.volume = 1f;
					MyAudio.Play();
					DigPhase++;
		...
		if (FakingReaction)
		{
			CharacterAnimation.CrossFade("f02_scaredIdle_01");
			if (Input.GetButtonUp("Y"))
			{
				FakingReaction = false;
				CanMove = true;
			}
		}
		if (CanMoveTimer > 0f)
		...
		if (SithAttacking)
		{
			if (!SithRecovering)
			{
				...
				else if (Pod.activeInHierarchy || Armor[20].activeInHierarchy)
				{
					...
					{
						Object.Instantiate(SithHitbox, base.transform.position + base.transform.forward * 1.5f + base.transform.up, base.transform.rotation).GetComponent<SithBeamScript>().Damage = 20f;
						SithAttacks++;
						if (SithCombo < 2)
						{
		...
		if (Finisher)
		{
			...
			else if (CharacterAnimation["f02_banchoFinisher_00"].time >= 1.66666663f)
			{
	...
	private void UpdateAttacking()
	{
		...
		else if (RoofPush)
		{
			...
			if (CharacterAnimation["f02_roofPushA_00"].time > 4.33333349f)
			{
			...
			else if (CharacterAnimation["f02_roofPushA_00"].time > 2.16666675f && TargetStudent.Schoolwear == 1 && !TargetStudent.ClubAttire && !Shoes[0].activeInHierarchy)
			{
		...
		else
		{
			...
			if (EquippedWeapon.WeaponID == 7)
			{
				...
				if (AttackPhase == 1)
				{
					if (CharacterAnimation["f02_buzzSawKill_A_00"].time > 0.333333343f)
					{
				...
				else if (AttackPhase < 6 && CharacterAnimation["f02_buzzSawKill_A_00"].time > 0.333333343f * (float)AttackPhase)
				{
	...
	private void UpdateDebugFunctionality()
	{
		...
		{
			if (!Aiming && CanMove && Time.timeScale > 0f && !StudentManager.TutorialActive && Input.GetKeyDown(KeyCode.Escape))
			{
	...
	private void LateUpdate()
	{
		...
		if (CanMove && sanity < 33.333f && !NearSenpai)
		{
	...
	{
		if (!StudentManager.TutorialActive && other.gameObject.name == "BloodPool(Clone)" && other.transform.localScale.x > 0.3f)
		{
	...
	public void ClubAccessory()
	{
		...
		if (!CensorSteam[0].activeInHierarchy && Club > ClubType.None && ClubAccessories[(int)Club] != null)
		{
