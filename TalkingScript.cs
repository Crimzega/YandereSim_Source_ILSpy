...
public class TalkingScript : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (S.Interaction == StudentInteractionType.Idle)
		{
			if (!Fake)
			{
				if (S.Yandere.TargetStudent != null && S.StudentID == 10 && S.FollowTarget != null && S.FollowTarget.Routine)
				{
			...
			if (S.TalkTimer == 3f)
			{
				if (S.Club != ClubType.Delinquent)
				{
					...
					if (!ConversationGlobals.GetTopicLearnedByStudent(19, S.StudentID))
					{
						...
						ConversationGlobals.SetTopicLearnedByStudent(19, S.StudentID, value: true);
					}
				}
				else
				{
		...
		else if (S.Interaction == StudentInteractionType.FollowingPlayer)
		{
			if (S.TalkTimer == 2f)
			{
				if (S.Club != ClubType.Delinquent)
				{
					...
					{
						if (S.Follower != null && S.Follower.CurrentAction == StudentActionType.Follow && !S.Follower.Distracting && !S.Follower.GoAway && !S.Follower.EatingSnack)
						{
							flag5 = true;
						}
					...
					if ((S.Clock.HourTime > 8f && S.Clock.HourTime < 13f) || (S.Clock.HourTime > 13.375f && S.Clock.HourTime < 15.5f) || (S.StudentID == S.StudentManager.RivalID && flag5) || (S.StudentID == S.StudentManager.RivalID && flag6) || SchoolGlobals.SchoolAtmosphere <= 0.5f)
					{
						S.CharacterAnimation.CrossFade(S.GossipAnim);
		...
		else if (S.Interaction == StudentInteractionType.GoingAway)
		{
			if (S.TalkTimer == 3f)
			{
				if (S.Club != ClubType.Delinquent)
				{
					if ((S.Clock.HourTime > 8f && S.Clock.HourTime < 13f) || (S.Clock.HourTime > 13.375f && S.Clock.HourTime < 15.5f) || SchoolGlobals.SchoolAtmosphere <= 0.5f)
					{
		...
		else if (S.Interaction == StudentInteractionType.TakingSnack)
		{
			...
			if (S.TalkTimer == 5f)
			{
				...
				else if (S.Fed || S.Club == ClubType.Council || flag7 || S.StudentID == 22)
				{
					S.CharacterAnimation.CrossFade(S.GossipAnim);
					S.Subtitle.UpdateLabel(SubtitleType.RejectFood, 0, 3f);
					S.IgnoreFoodTimer = 10f;
		...
		if (S.StudentID == 41 && !S.DialogueWheel.ClubLeader && S.Interaction != StudentInteractionType.ClubUnwelcome && !S.StudentManager.Eighties && S.TalkTimer > 0f)
		{
