...
public class OsanaMondayLunchEventScript : MonoBehaviour
{
	...
	private void Update()
	{
		if (Phase == 0)
		{
			if (Frame > 0 && StudentManager.Students[RivalID] != null && StudentManager.Students[RivalID].enabled && StudentManager.Students[1].gameObject.activeInHierarchy && Clock.Period == 3)
			{
		...
		if (Clock.Period > 3 || Senpai.Alarmed || Rival.Alarmed || Rival.Wet)
		{
