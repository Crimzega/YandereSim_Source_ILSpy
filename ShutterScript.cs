...
public class ShutterScript : MonoBehaviour
{
			...
			if (PhotoIcons.activeInHierarchy && !Snapping && !TextMessages.gameObject.activeInHierarchy)
			{
				...
				if (Input.GetButtonDown("A"))
				{
					if (!Yandere.RivalPhone)
					{
						...
						if (FreeSpace)
						{
							...
							PlayerGlobals.SetPhoto(Slot, value: true);
							if (flag2)
							{
							...
							if (flag3)
							{
								PlayerGlobals.SetSenpaiPhoto(Slot, value: true);
								PlayerGlobals.SenpaiShots++;
								Yandere.Inventory.SenpaiShots++;
							}
							if (AirGuitarShot)
							{
								TaskGlobals.SetGuitarPhoto(Slot, value: true);
								TaskManager.UpdateTaskStatus();
							}
							if (KittenShot)
							{
								TaskGlobals.SetKittenPhoto(Slot, value: true);
								TaskManager.UpdateTaskStatus();
							}
							if (HorudaShot)
							{
								TaskGlobals.SetHorudaPhoto(Slot, value: true);
								TaskManager.UpdateTaskStatus();
							}
				...
				if (!Yandere.RivalPhone && Input.GetButtonDown("X"))
				{
					...
					if (!InfoX.activeInHierarchy)
					{
						...
						if (!StudentGlobals.GetStudentPhotographed(Student.StudentID))
						{
							Yandere.Inventory.PantyShots++;
						}
						StudentGlobals.SetStudentPhotographed(Student.StudentID, value: true);
	...
	public void CheckPhoto()
	{
		...
		NotFace = false;
		Skirt = false;
		Transform transform = ((!Yandere.Aiming) ? Palm : SmartphoneCamera.transform);
	...
	public void ResumeGameplay()
	{
		...
		Physics.Raycast(Vector3.zero, Vector3.forward, out hit, float.PositiveInfinity, OnlyPhotography);
	}
