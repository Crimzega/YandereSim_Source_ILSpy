...
public class ClockScript : MonoBehaviour
{
	...
	public void Update()
	{
		...
		if (!StopTime)
		{
			if (Period == 3)
			{
				PresentTime += Time.deltaTime * 0.0166666675f * TimeSpeed * 0.5f;
			}
			else
			{
				PresentTime += Time.deltaTime * 0.0166666675f * TimeSpeed;
			}
		}
		HourTime = PresentTime / 60f;
		Hour = Mathf.Floor(PresentTime / 60f);
		...
		else if (HourTime < 13.5f)
		{
			if (Period < 3)
			{
				...
				if (!StudentManager.Eighties && DateGlobals.Week == 1 && !StudentManager.MissionMode)
				{
		...
		if (!Horror)
		{
			...
			{
				for (int j = 1; j < Bathroom.Length; j++)
				{
					if (Bathroom[j].bounds.Contains(Yandere.transform.position))
					{
						if (!BathroomLight[j].enabled)
						{
							BathroomDim = 0.5f;
						}
						else
			...
			if (PresentTime > 930f)
			{
				...
				MainLight.color = new Color(1f - 367f / (906f * (float)Math.E) * DayProgress, 1f - 0.403921545f * DayProgress, 1f - 181f / 255f * DayProgress);
				RenderSettings.ambientLight = new Color(1f - 367f / (906f * (float)Math.E) * DayProgress - (1f - AmbientLightDim) * (1f - DayProgress), 1f - 0.403921545f * DayProgress - (1f - AmbientLightDim) * (1f - DayProgress), 1f - 181f / 255f * DayProgress - (1f - AmbientLightDim) * (1f - DayProgress));
				SkyboxColor = new Color(1f - 367f / (906f * (float)Math.E) * DayProgress - 0.5f * (1f - DayProgress), 1f - 0.403921545f * DayProgress - 0.5f * (1f - DayProgress), 1f - 181f / 255f * DayProgress - 0.5f * (1f - DayProgress));
		...
		if (Yandere.CameraEffects.Streaks.color.a > 0f || Yandere.CameraEffects.MurderStreaks.color.a > 0f || Yandere.NearSenpai || Input.GetButtonDown("B"))
		{
			EndTimeSkip();
		}
	}
