...
public class YakuzaMenuScript : MonoBehaviour
{
	...
	private void Start()
	{
		MoneyLabel.text = "$" + PlayerGlobals.Money;
		...
		if (GameGlobals.YakuzaPhase == 0 || !HomeGlobals.Night)
		{
		...
		{
			PrisonerLabel.text = "Come back when you have a prisoner in your basement.";
		}
		else
		{
			PrisonerLabel.text = "You currently have a prisoner in your basement.";
		}
	}
		...
		if (Show)
		{
			...
			else if (Menu == 4)
			{
				...
				if (KidnappingMenu.alpha == 1f)
				{
					if (!RansomConfirmationWindow.activeInHierarchy)
					{
						if (Input.GetButtonDown("A"))
						{
							if (SchoolGlobals.KidnapVictim > 0)
							{
								...
								RansomConfirmationLabel.text = "Give the kidnapped student in your basement to the yakuza in exchange for $" + Ransom[SchoolGlobals.KidnapVictim] + "?";
								PromptBar.Show = false;
							}
						}
						else if (Input.GetButtonDown("B"))
					...
					{
						AudioSource.PlayClipAtPoint(ContrabandPurchase, Yandere.MainCamera.transform.position);
						StudentGlobals.SetStudentKidnapped(SchoolGlobals.KidnapVictim, value: false);
						StudentGlobals.SetStudentMissing(SchoolGlobals.KidnapVictim, value: false);
						StudentGlobals.SetStudentRansomed(SchoolGlobals.KidnapVictim, value: true);
						StudentGlobals.SetStudentBroken(SchoolGlobals.KidnapVictim, value: true);
						PlayerGlobals.Money += Ransom[SchoolGlobals.KidnapVictim];
						MoneyLabel.text = "$" + PlayerGlobals.Money;
						UpdateRansomPortraits();
