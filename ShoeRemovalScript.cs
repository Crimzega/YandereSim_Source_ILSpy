		...
		if (!Student.DiscCheck && !Student.Dying && !Student.Alarmed && !Student.Splashed && !Student.TurnOffRadio)
		{
			...
			if (Phase == 1)
			{
				if (Student.CharacterAnimation[RemovalAnim].time >= 0.833333f)
				{
					...
					if (Student.StudentID == Student.StudentManager.RivalID && !Student.StudentManager.MissionMode && !GameGlobals.AlphabetMode && !GameGlobals.Eighties && DateGlobals.Week == 1)
					{
