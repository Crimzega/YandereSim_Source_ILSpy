...
public class NoteLockerScript : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (Student != null && ((Student.Routine && Student.Phase == 2) || (Student.Routine && Student.Phase == 8) || Student.SentToLocker) && Vector3.Distance(base.transform.position, Student.transform.position) < 1.5f && !Student.InEvent)
		{
		...
		if (Timer == 0f)
		{
			...
			{
				Student.FollowTargetDestination.localPosition = new Vector3(1f, 0f, 0f);
			}
			Phase = 1;
		}
