...
namespace YandereSimulator.Yancord
{
	public class YancordManager : MonoBehaviour
	{
		...
		private void SpawnAll()
		{
			...
			{
				MessageScript item = Object.Instantiate(MessagePrefab, new Vector3(0f, Messages[i - 1].transform.position.y - ((float)Messages[i - 1].MessageLabel.height * 0.00167239446f + Distance * 0.00167239446f), 0f), Quaternion.identity, ConversationParent);
				Messages.Add(item);
		...
		private void SpawnChatMessage()
		{
			if (Messages[currentPhase].transform.position.y < -400f || Messages[currentPhase].transform.localPosition.y - (float)Messages[currentPhase].MessageLabel.height < -400f)
			{
				if (!Messages[currentPhase].gameObject.activeInHierarchy)
				{
					...
					Messages[currentPhase].transform.position = new Vector3(0f, num * 0.00167239446f, 0f);
					for (int num2 = currentPhase - 1; num2 >= 0; num2--)
					{
						Messages[num2].transform.position = new Vector3(0f, Messages[num2 + 1].transform.position.y + ((float)Messages[num2].MessageLabel.height * 0.00167239446f + Distance * 0.00167239446f), 0f);
					}
					for (int i = 1; i < Messages.Count; i++)
					{
						Messages[i].transform.position = new Vector3(0f, Messages[i - 1].transform.position.y - ((float)Messages[i - 1].MessageLabel.height * 0.00167239446f + Distance * 0.00167239446f), 0f);
					}
				}
			}
			else if (!Messages[currentPhase].gameObject.activeInHierarchy)
			{
				...
				{
					Messages[j].transform.position = new Vector3(0f, Messages[j - 1].transform.position.y - ((float)Messages[j - 1].MessageLabel.height * 0.00167239446f + Distance * 0.00167239446f), 0f);
				}
			}
		}
		...
		private void JoinServer()
		{
			...
			CreateNewServer.position = new Vector3(CreateNewServer.position.x, 101f / 270f, CreateNewServer.position.z);
