	...
	private void Update()
	{
		...
		if (FadeOut)
		{
			...
			if (Phase == 1)
			{
				...
				{
					Yandere.Follower.CharacterAnimation.Play("stretch_00");
					Yandere.transform.position = YandereSpot.position;
					Yandere.transform.eulerAngles = YandereSpot.eulerAngles;
					Yandere.Follower.transform.position = SuitorSpot.position;
					Yandere.Follower.transform.eulerAngles = SuitorSpot.eulerAngles;
			...
			else
			{
				...
				if (TraitID == 2)
				{
					YandereChair.transform.localPosition = new Vector3(YandereChair.transform.localPosition.x, YandereChair.transform.localPosition.y, -0.333333343f);
					SuitorChair.transform.localPosition = new Vector3(SuitorChair.transform.localPosition.x, SuitorChair.transform.localPosition.y, -0.333333343f);
				}
		...
		{
			DatingGlobals.SetSuitorTrait(TraitID, DatingGlobals.GetSuitorTrait(TraitID) + 1);
