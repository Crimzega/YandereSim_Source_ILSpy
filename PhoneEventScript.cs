	...
	private void Update()
	{
		if (!Clock.StopTime && EventCheck)
		{
			...
			else if (Clock.HourTime > EventTime)
			{
				...
				if (EventStudent != null && !EventStudent.InEvent && !EventStudent.Meeting && EventStudent.DistanceToDestination < 1f && !EventStudent.Phoneless)
				{
					...
					if (Timer > 1f)
					{
						...
						if (EventStudentID == 11)
						{
							...
							if (EventFriend != null && EventFriend.CurrentAction == StudentActionType.Follow)
							{
		...
		if (Clock.HourTime > EventTime + 0.5f || EventStudent.WitnessedMurder || EventStudent.Splashed || EventStudent.Alarmed || EventStudent.Dodging || EventStudent.Dying || !EventStudent.Alive)
		{
		...
		if (!EventStudent.Pathfinding.canMove)
		{
			...
			else if (EventPhase == 2)
			{
				...
				if (Timer > 1.5f)
				{
					...
					EventStudent.SmartPhone.transform.localPosition = new Vector3(-0.015f, -0.005f, -0.015f);
			...
			else if (EventPhase == 3)
			{
				...
				if (Timer > CurrentClipLength)
				{
					...
					Debug.Log(EventStudent.Name + " has been given a pathfinding speed of 4.");
					Timer = 0f;
					EventPhase++;
				}
			}
			...
			if (EventPhase == 11 && num < 5f)
			{
				if (EventStudentID == 30)
				{
					if (!EventGlobals.Event2)
					{
						...
						ConversationGlobals.SetTopicLearnedByStudent(25, EventStudentID, value: true);
					}
				}
		...
		if ((!EventStudent.Pathfinding.canMove && EventPhase <= 3) || !(EventFriend != null) || EventFriend.CurrentAction != StudentActionType.Follow || EventPhase <= 3)
		{
			return;
		}
